<template>
    <fgp-task name="installFrontend">
        ...
        <template v-slot:description>
            Depending on the value of the <fgp-property-link name="yarnEnabled" /> property, this task issues either a
            <fgp-code>npm install</fgp-code> command or a <fgp-code>yarn install</fgp-code> command, by default.
            Consequently, the command shall install/update dependencies, and perform any additional actions as described
            in the <fgp-code>package.json</fgp-code> file located in the directory pointed by the
            <fgp-property-link name="packageJsonDirectory" /> property. Optionally, this command may be customized with
            the <fgp-property-link name="installScript" /> property (e.g. to run a <fgp-code>npm ci</fgp-code> command
            instead). This task depends on the <fgp-task-link name="installNode" /> task, and optionally on the
            <fgp-task-link name="installYarn" /> task if the <fgp-property-link name="yarnEnabled" /> property is
            <fgp-code>true</fgp-code>.
