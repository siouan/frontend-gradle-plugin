...
import org.siouan.frontendgradleplugin.CheckTask;
...
public class CheckFuncTest {
    ...
    @Test
    public void shouldFailCheckingWhenNodeDistributionCannotBeDownloaded() throws IOException {
        FunctionalTestHelper.createBuildFile(projectDirectory, Collections.singletonMap("nodeVersion", "0.76.34"));

        final BuildResult result = runGradleAndExpectFailure(projectDirectory, GRADLE_CHECK_TASK);

        assertTaskOutcome(result, NodeInstallTask.DEFAULT_NAME, TaskOutcome.FAILED);
    }

    @Test
    public void shouldFailCheckingWhenYarnDistributionCannotBeDownloaded() throws IOException {
